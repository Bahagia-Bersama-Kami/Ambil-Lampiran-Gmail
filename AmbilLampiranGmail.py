...
def download_attachments(service, cfg):
    ...
    try:
        results = service.users().messages().list(userId='me', q=cfg['query']).execute()
        ...
        print(f"Gmail API menemukan {len(messages)} percakapan (kandidat).")
        print("Memulai pengecekan tanggal presisi...")
        ...
        for msg in messages:
            ...
            for part in parts:
                ...
                if filename:
                    ...
                    clean_filename = f"{date_prefix}_{msg['id']}_{filename}"
                    path = os.path.join(folder, clean_filename)
                    
                    with open(path, 'wb') as f:
                        f.write(file_data)
                    
                    print(f"[OK] {date_prefix} | {filename}")
                    count_download += 1
        
        print(f"\nSelesai! {count_download} file diunduh. {count_skip} email dilewati karena tanggal tidak sesuai.")
